package com.novashen.widget.download
...
import com.novashen.util.FilePathUtil
import com.novashen.util.FileUtil
import com.novashen.util.OkHttpDns
import com.novashen.util.SSLUtil
import com.novashen.widget.R
import okhttp3.Call
    ...
    private fun getViewIntent(): Intent {
        return if (DownLoadUtil.isDownloadFolderUriAccessible(context)) {
            ...
            val intentUri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                FileProvider.getUriForFile(context, "com.novashen.widget.download.downloadFileProvider", file)
            } else {
                Uri.fromFile(file)
            }
